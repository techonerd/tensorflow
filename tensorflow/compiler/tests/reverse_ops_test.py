...
class ReverseOpsTest(xla_test.XLATestCase):
  ...
  def testReverseMoreThanOneDim(self):
    ...
    # index.
    for revdims in itertools.chain.from_iterable(itertools.combinations(range(-offset,
                                     len(shape) - offset), k) for k in range(2,
                         len(shape) + 1) for offset in range(len(shape))):
